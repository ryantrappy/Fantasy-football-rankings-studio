...
function validate(input: unknown): EspnCredentials {
  ...
  if (!result.success)
    throw new HttpException(400, 'Enter both ESPN cookie values: espn_s2 and a valid SWID.');
  return result.data;
}
function encryptionKey() {
  ...
  if (!value || !/^[a-f\d]{64}$/i.test(value))
    throw new HttpException(
      503,
      'ESPN credential storage is not configured. Contact the administrator.',
    );
...
export function decryptCredentials(owner: string, value: string): EspnCredentials {
  ...
  try {
    ...
    throw new HttpException(
      503,
      'Saved ESPN credentials could not be read. Save them again in ESPN settings.',
    );
  }
}
...
async function storage<T>(action: () => Promise<T>): Promise<T> {
  try {
    ...
    throw new HttpException(503, 'ESPN credential storage is unavailable. Please try again.');
  }
}
export async function getEspnCredentialStatus(owner: string): Promise<EspnCredentialStatus> {
  ...
  return {
    configured: !!record?.encryptedCredentials,
...
export async function getEspnCredentials(owner: string): Promise<EspnCredentials | undefined> {
  ...
  return record?.encryptedCredentials
    ? decryptCredentials(owner, record.encryptedCredentials)
    : undefined;
}
export async function saveEspnCredentials(
  owner: string,
  input: unknown,
): Promise<EspnCredentialStatus> {
  const encryptedCredentials = encryptCredentials(owner, validate(input));
  await storage(() =>
    credentialModel
      .updateOne(
        { _id: owner },
        { $set: { encryptedCredentials, onboardingComplete: true } },
        { upsert: true },
      )
      .exec(),
  );
...
export async function removeEspnCredentials(owner: string): Promise<EspnCredentialStatus> {
  await storage(() =>
    credentialModel
      .updateOne(
        { _id: owner },
        { $unset: { encryptedCredentials: 1 }, $set: { onboardingComplete: true } },
        { upsert: true },
      )
      .exec(),
  );
...
export async function skipEspnSetup(owner: string): Promise<EspnCredentialStatus> {
  await storage(() =>
    credentialModel
      .updateOne({ _id: owner }, { $set: { onboardingComplete: true } }, { upsert: true })
      .exec(),
  );
